...
FetchResponse.fromBytes = function(bytes){
  ...
  vars = binary.parse(bytes)
      .word32bu('length')
      .word16bs('error')
      .buffer('body', expectedLength - 2)  // 2 for error code bytes
      .vars;


  // TODO error if length is null, or err is not an acceptible #.
  var messages = parseMessages(vars.body);
  this.bytesLengthVal = bytes.length;

  return new FetchResponse(vars.error, messages);
