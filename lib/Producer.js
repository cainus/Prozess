...
var strategy = require('./PartitionerStrategies');
...
var Producer = function(topic, options){
  ...
  options = options || {};
  this.topic = topic;
  this.partitioner = options.partitioner || new Partitioner(
      strategy.fixedPartitioner(options.partition || 0)
  );
...
Producer.prototype.send = function(messages, cb) {
  ...
  var partitionedMessages = this.partitioner.partition(toListOfMessages(toArray(messages)));
  var _ref = _.keys(partitionedMessages);
  for (var _i = 0, _len = _ref.length; _i < _len; _i++) {
    var partition = _ref[_i];
    var request = new ProduceRequest(this.topic, parseInt(partition), partitionedMessages[partition]);
